...
def test_readme():
    ...
    # signature: G2Element = PopSchemeMPL.sign(sk, message)
    #
    # ok: bool = PopSchemeMPL.verify(pk, message, signature)
    # assert ok
    #
    ...
    # sig2: G2Element = PopSchemeMPL.sign(sk2, message2)
    #
    # agg_sig: G2Element = PopSchemeMPL.aggregate([sig1, sig2])
    #
    # ok = AugSchemeMPL.aggregate_verify([pk1, pk2], [message, message2], agg_sig)
    # assert ok

    seed = bytes([3]) + seed[1:]
    sk3: PrivateKey = PopSchemeMPL.key_gen(seed)
    pk3: G1Element = sk3.get_g1()
    # message3: bytes = bytes([100, 2, 254, 88, 90, 45, 23])
    # sig3: G2Element = PopSchemeMPL.sign(sk3, message3)
    #
    # agg_sig_final: G2Element = PopSchemeMPL.aggregate([agg_sig, sig3])
    # ok = PopSchemeMPL.aggregate_verify(
    ...
    # print("public key size: ", sys.getsizeof(pk1))
    # print("secret key size: ", sys.getsizeof(sk1))
    # print("Signature Size: ", sys.getsizeof(pop_sig1))
    #
    # print("public key size: ", sys.getsizeof(pk2))
    # print("secret key size: ", sys.getsizeof(sk2))
    # print("Signature Size: ", sys.getsizeof(pop_sig2))
    #
    # print("public key size: ", sys.getsizeof(pk3))
    # print("secret key size: ", sys.getsizeof(sk3))
    # print("Signature Size: ", sys.getsizeof(pop_sig3))
    ...
    sig_aggregation_start_time = int(time.time())
    pop_sig_agg: G2Element = PopSchemeMPL.aggregate([pop_sig1, pop_sig2, pop_sig3])
    sig_aggregation_end_time = int(time.time())
    ...
    master_sk: PrivateKey = PopSchemeMPL.key_gen(seed)
    child: PrivateKey = PopSchemeMPL.derive_child_sk(master_sk, 152)
    grandchild: PrivateKey = PopSchemeMPL.derive_child_sk(child, 952)
    assert grandchild is not None

    master_pk: G1Element = master_sk.get_g1()
    child_u: PrivateKey = PopSchemeMPL.derive_child_sk_unhardened(master_sk, 22)
    grandchild_u: PrivateKey = PopSchemeMPL.derive_child_sk_unhardened(child_u, 0)

    child_u_pk: G1Element = PopSchemeMPL.derive_child_pk_unhardened(master_pk, 22)
    grandchild_u_pk: G1Element = PopSchemeMPL.derive_child_pk_unhardened(child_u_pk, 0)
