...
def get_public_key_size(public_key):
  """Gets the size of the public key in bytes."""
  public_key_bytes = base64.b64decode(public_key)
  return len(public_key_bytes)
...
def test_schemes():
    ...
    # fmt: on
    msg = b"This is a text message that is 500 bytes long. This is a text message that is 500 bytes long. This is a text message that is 500 bytes long. This is a text message that is 500 bytes long. This is a text message that is 500 bytes long. This is a text message that is 500 bytes long. This is a text message that is 500 bytes long. This is a text message that is 500 byIt is being encrypted using a strong encryption algorithm. The encrypted message will be difficult to decrypt without the correct key."
    ...
    seed = bytes([1]) + seed[1:]
    sk1 = PopSchemeMPL.key_gen(seed)
    pk1 = sk1.get_g1()
    seed = bytes([2]) + seed[1:]
    sk2 = PopSchemeMPL.key_gen(seed)
    pk2 = sk2.get_g1()
    seed = bytes([3]) + seed[1:]
    sk3 = PopSchemeMPL.key_gen(seed)
    pk3 = sk3.get_g1()
    ...
    print("Public Key 1 : ")
    print(pk1)
    size = len(base64.b64decode(pk1))
    print("Size of Public Key: ", size)

    print("Public Key 2 : ")
    print(pk2)
    size = len(base64.b64decode(pk2))
    print("Size of Public Key: ", size)

    print("Public Key 3 : ")
    print(pk3)
    size = len(base64.b64decode(pk3))
    print("Size of Public Key: ", size)

    agg_pk = pk1 + pk2 + pk3
    print("Aggregated Public Keys: ")
    print(agg_pk)
    # if Scheme is AugSchemeMPL:
    #     sig1 = Scheme.sign(sk1, msg, agg_pk)
    #     sig2 = Scheme.sign(sk2, msg, agg_pk)
    # else:
    print("Aggregating same message")
    sig1 = PopSchemeMPL.sign(sk1, msg)
    print("First Signature Created")
    print(sig1)
    sig2 = PopSchemeMPL.sign(sk2, msg)
    print("Second Signature Created")
    print(sig2)
    sig3 = PopSchemeMPL.sign(sk3, msg)
    print("Third Signature Created")
    print(sig3)

    size = sys.getsizeof(sig1)
    print("Signature 1 Size: ", size)
    size = sys.getsizeof(sig2)
    print("Signature 2 Size: ", size)
    size = sys.getsizeof(sig3)
    print("Signature 3 Size: ", size)

    agg_sig = PopSchemeMPL.aggregate([sig1, sig2, sig3])
    print("Aggregated Signature = ")
    print(agg_sig)

    assert PopSchemeMPL.verify(agg_pk, msg, agg_sig)
    print("Aggregated Signature Verified Successfully")

    # Aggregate different message
    # print("Aggregating different message")
    # sig1 = PopSchemeMPL.sign(sk1, msg)
    # print("First Signature Created")
    # print(sig1)
    # sig2 = PopSchemeMPL.sign(sk2, msg2)
    # print("Second Signature Created")
    # print(sig2)
    # agg_sig = PopSchemeMPL.aggregate([sig1, sig2])
    # print("Aggregated Signature = ")
    # print(agg_sig)
    # assert PopSchemeMPL.aggregate_verify([pk1, pk2], [msg, msg2], agg_sig)
    # print("Aggregated Signature Verified Successfully")
